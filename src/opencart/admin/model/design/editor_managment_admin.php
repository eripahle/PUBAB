...
class ModelDesignEditorManagmentAdmin extends Model {

    public function addEditor($data,$file) {    
//        print_r($data);
//       print_r($file);
        $this->event->trigger('pre.admin.product.add', $data);                
        $sql = "INSERT INTO " . DB_PREFIX .
                "editor_info SET info = '" . $this->db->escape($data['description'])
                . "', editor_id = '" . $this->db->escape($data['editor_id']);
        ...
        if (in_array($file_ext, $allowed_ext) === true) {
            if ($file_size < 1044070000) {
                $lokasi = 'image/editor/' . $nama . '.' . $file_ext;
                $lokasisimpan = '../image/editor/' . $nama . '.' . $file_ext;
...
//        echo $sql;
    ...
    public function deleteEditor($id) {
       $query=$this->db->query("DELETE FROM " . DB_PREFIX . "editor_info WHERE editor_id = " . (int)$id . "");
        if ($query) {
            return true;
        }
    }
    ...
    public function getEditors() {        
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "editor," . DB_PREFIX . "editor_info WHERE " . DB_PREFIX . "editor.editor_id = " . DB_PREFIX . "editor_info.editor_id");         
