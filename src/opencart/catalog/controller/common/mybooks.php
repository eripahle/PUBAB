...
class ControllerCommonMybooks extends Controller {
	...
	//function untuk mengedi buku hasil editing yang di lakukan oleh editor
	public function editListBooks() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			if(!empty($this->request->files['book_edit']['name'])){
				if($this->checkValidRequestDrafBookEditor()){
					...
					$books=$this->model_catalog_productbooks->getBooks($this->request->get['product_id']);
					...
					move_uploaded_file($this->request->files['book_edit']['tmp_name'], DIR_BOOK."/".$lokasi_book);
				}
				$statusEditing=true;
		...
		$this->getFormEditingbooks();
	}

	public function editDesignBooks() {
		...
		$this->getFormDesignbooks();
	}

	public function editMyBooks() {
		if(!$this->customer->isLogged()){
			$this->response->redirect($this->url->link('account/login', ' ', 'SSL'));
		}
		$this->load->language('product/mybooks');
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$product_specials = $this->model_catalog_productbooks->getProductSpecials($result['product_id']);
		...
		$data['sort_status_author'] = $this->url->link('common/mybooks',  '&sort=p.status1' . $url, 'SSL');
		$data['sort_status_editor'] = $this->url->link('common/mybooks',  '&sort=p.status2' . $url, 'SSL');
		$data['sort_status_management'] = $this->url->link('common/mybooks',  '&sort=p.status3' . $url, 'SSL');
	...
	public function getEditingList() {
		...
		$this->document->setTitle($this->language->get('heading_title_list_edit'));
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title_list_edit'),
			'href' => $this->url->link('catalog/product', ' ', 'SSL')
		);
		...
		$this->response->setOutput($this->load->view('default/template/common/editingbooks.tpl', $data));
	}
	...
	public function getDesignList() {
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'status2'    => ($result['status2']) ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
				'status3'    => ($result['status3']) ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
				'edit'       => $this->url->link('common/mybooks/editDesignBooks', '&product_id=' . $result['product_id'] . $url, 'SSL'),
				//'download'   => $this->url->link('catalog/product/downloadBooks', '&product_id=' . $result['product_id'] . $url, 'SSL')
				'download'   => $download,
				'totalSelling'=>$totalSelling,
				'your_royalty'=>$totalSelling*$result['royalty']
		...
		$this->response->setOutput($this->load->view('default/template/common/designbooks.tpl', $data));
	}

	protected function getFormMybooks() {
		...
		if (isset($this->error['color_page'])) {
			$data['error_color_page'] = $this->error['color_page'];
		} else {
			$data['error_color_page'] = '';
		}

		if (isset($this->error['bw_page'])) {
		...
		if (!isset($this->request->get['product_id'])) {
			...
			$data['action'] = $this->url->link('common/mybooks/editMyBooks',  '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		}
	...
	protected function getFormEditingbooks() {
		//$this->load->language('product/editmybook');
		$data['heading_title'] = $this->language->get('heading_title');
		...
		$data['cancel'] = $this->url->link('common/mybooks/getEditingList', ' ', 'SSL');
		...
		$this->response->setOutput($this->load->view('default/template/common/form_editingbooks.tpl', $data));
	}

	protected function getFormDesignbooks() {
		//$this->load->language('product/editmybook');
		$data['heading_title'] = $this->language->get('heading_title');
		...
		if (isset($this->request->post['desc_request_design'])) {
			$data['desc_request_design'] = $this->request->post['desc_request_design'];
		} elseif (!empty($product_info)) {
			$data['desc_request_design'] = $product_info['desc_design'];
		} else {
			$data['desc_request_design'] = " ";
		}
	...
	protected function checkValidRequestDrafimage() {
	        $dot = '.';
	...
	protected function validateForm() {
		...
		if(empty($this->request->files['sample_script']['name'])){
			$this->error['sample_book']=$this->language->get('error_book');
		}
		...
		if ($this->request->post['bw_page']< 0 || $this->request->post['bw_page']==null ){
